...
use lib qw(../../../.);  
 ...
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::AllocateAddressRequest
      ...
      eval {
              my $response = $service->allocateAddress($request);
                ...
                if ($response->isSetAllocateAddressResult()) { 
                    print("            AllocateAddressResult\n");
                    my $allocateAddressResult = $response->getAllocateAddressResult();
                    if ($allocateAddressResult->isSetPublicIp()) 
                    {
                        print("                PublicIp\n");
                        print("                    " . $allocateAddressResult->getPublicIp() . "\n");
                    }
                } 

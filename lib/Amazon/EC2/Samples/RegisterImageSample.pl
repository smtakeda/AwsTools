...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::RegisterImageRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::RegisterImageRequest
 ...
 # invokeRegisterImage($service, $request);
    ...
    #
  sub invokeRegisterImage {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->registerImage($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        RegisterImageResponse\n");
                ...
                if ($response->isSetRegisterImageResult()) { 
                    print("            RegisterImageResult\n");
                    my $registerImageResult = $response->getRegisterImageResult();
                    if ($registerImageResult->isSetImageId()) 
                    {
                        print("                ImageId\n");
                        print("                    " . $registerImageResult->getImageId() . "\n");
                    }
                } 

...
use lib qw(../../../.);  
 ...
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeSubnetsRequest
      ...
      eval {
              my $response = $service->describeSubnets($request);
                ...
                if ($response->isSetDescribeSubnetsResult()) { 
                    print("            DescribeSubnetsResult\n");
                    my $describeSubnetsResult = $response->getDescribeSubnetsResult();
                    ...
                    foreach (@$subnetList) {
                        ...
                        {
                            print("                    SubnetId\n");
                            print("                        " . $subnet->getSubnetId() . "\n");
                        }
                        if ($subnet->isSetSubnetState()) 
                        {
                            print("                    SubnetState\n");
                            print("                        " . $subnet->getSubnetState() . "\n");
                        }
                        if ($subnet->isSetVpcId()) 
                        {
                            print("                    VpcId\n");
                            print("                        " . $subnet->getVpcId() . "\n");
                        }
                        if ($subnet->isSetCidrBlock()) 
                        {
                            print("                    CidrBlock\n");
                            print("                        " . $subnet->getCidrBlock() . "\n");
                        }
                        if ($subnet->isSetAvailableIpAddressCount()) 
                        {
                            print("                    AvailableIpAddressCount\n");
                            print("                        " . $subnet->getAvailableIpAddressCount() . "\n");
                        }
                        if ($subnet->isSetAvailabilityZone()) 
                        {
                            print("                    AvailabilityZone\n");
                            print("                        " . $subnet->getAvailabilityZone() . "\n");
                        }
                    }
                } 

...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeCustomerGatewaysRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeCustomerGatewaysRequest
 ...
 # invokeDescribeCustomerGateways($service, $request);
    ...
    #
  sub invokeDescribeCustomerGateways {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->describeCustomerGateways($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        DescribeCustomerGatewaysResponse\n");
                ...
                if ($response->isSetDescribeCustomerGatewaysResult()) { 
                    print("            DescribeCustomerGatewaysResult\n");
                    my $describeCustomerGatewaysResult = $response->getDescribeCustomerGatewaysResult();
                    my $customerGatewayList = $describeCustomerGatewaysResult->getCustomerGateway();
                    foreach (@$customerGatewayList) {
                        my $customerGateway = $_;
                        print("                CustomerGateway\n");
                        if ($customerGateway->isSetCustomerGatewayId()) 
                        {
                            print("                    CustomerGatewayId\n");
                            print("                        " . $customerGateway->getCustomerGatewayId() . "\n");
                        }
                        if ($customerGateway->isSetCustomerGatewayState()) 
                        {
                            print("                    CustomerGatewayState\n");
                            print("                        " . $customerGateway->getCustomerGatewayState() . "\n");
                        }
                        if ($customerGateway->isSetType()) 
                        {
                            print("                    Type\n");
                            print("                        " . $customerGateway->getType() . "\n");
                        }
                        if ($customerGateway->isSetIpAddress()) 
                        {
                            print("                    IpAddress\n");
                            print("                        " . $customerGateway->getIpAddress() . "\n");
                        }
                        if ($customerGateway->isSetBgpAsn()) 
                        {
                            print("                    BgpAsn\n");
                            print("                        " . $customerGateway->getBgpAsn() . "\n");
                        }
                    }
                } 

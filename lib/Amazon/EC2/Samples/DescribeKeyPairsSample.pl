...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeKeyPairsRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeKeyPairsRequest
 ...
 # invokeDescribeKeyPairs($service, $request);
    ...
    #
  sub invokeDescribeKeyPairs {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->describeKeyPairs($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        DescribeKeyPairsResponse\n");
                ...
                if ($response->isSetDescribeKeyPairsResult()) { 
                    print("            DescribeKeyPairsResult\n");
                    my $describeKeyPairsResult = $response->getDescribeKeyPairsResult();
                    my $keyPairList = $describeKeyPairsResult->getKeyPair();
                    foreach (@$keyPairList) {
                        my $keyPair = $_;
                        print("                KeyPair\n");

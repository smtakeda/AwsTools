...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeAddressesRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeAddressesRequest
 ...
 # invokeDescribeAddresses($service, $request);
    ...
    #
  sub invokeDescribeAddresses {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->describeAddresses($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        DescribeAddressesResponse\n");
                ...
                if ($response->isSetDescribeAddressesResult()) { 
                    print("            DescribeAddressesResult\n");
                    my $describeAddressesResult = $response->getDescribeAddressesResult();
                    my $addressList = $describeAddressesResult->getAddress();
                    foreach (@$addressList) {
                        my $address = $_;
                        print("                Address\n");
                        ...
                        {
                            print("                    InstanceId\n");
                            print("                        " . $address->getInstanceId() . "\n");
                        }
                        if ($address->isSetPublicIp()) 
                        {
                            print("                    PublicIp\n");
                            print("                        " . $address->getPublicIp() . "\n");
                        }
                    }
                } 

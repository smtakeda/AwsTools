...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeAvailabilityZonesRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeAvailabilityZonesRequest
 ...
 # invokeDescribeAvailabilityZones($service, $request);
    ...
    #
  sub invokeDescribeAvailabilityZones {
      ...
      eval {
              my $response = $service->describeAvailabilityZones($request);
                ...
                if ($response->isSetDescribeAvailabilityZonesResult()) { 
                    print("            DescribeAvailabilityZonesResult\n");
                    my $describeAvailabilityZonesResult = $response->getDescribeAvailabilityZonesResult();
                    my $availabilityZoneList = $describeAvailabilityZonesResult->getAvailabilityZone();
                    foreach (@$availabilityZoneList) {
                        my $availabilityZone = $_;
                        print("                AvailabilityZone\n");
                        if ($availabilityZone->isSetZoneName()) 
                        {
                            print("                    ZoneName\n");
                            print("                        " . $availabilityZone->getZoneName() . "\n");
                        }
                        if ($availabilityZone->isSetZoneState()) 
                        {
                            print("                    ZoneState\n");
                            print("                        " . $availabilityZone->getZoneState() . "\n");
                        }
                        if ($availabilityZone->isSetRegionName()) 
                        {
                            print("                    RegionName\n");
                            print("                        " . $availabilityZone->getRegionName() . "\n");
                        }
                        my $messageList  =  $availabilityZone->getMessage();
                        foreach (@$messageList) { 
                            my $message = $_;
                            print("                    Message\n");
                            print("                        " . $message);

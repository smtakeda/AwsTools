...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeSnapshotAttributeRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeSnapshotAttributeRequest
 ...
 # invokeDescribeSnapshotAttribute($service, $request);
    ...
    #
  sub invokeDescribeSnapshotAttribute {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->describeSnapshotAttribute($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        DescribeSnapshotAttributeResponse\n");
                ...
                if ($response->isSetDescribeSnapshotAttributeResult()) { 
                    print("            DescribeSnapshotAttributeResult\n");
                    my $describeSnapshotAttributeResult = $response->getDescribeSnapshotAttributeResult();
                    if ($describeSnapshotAttributeResult->isSetSnapshotAttribute()) { 
                        print("                SnapshotAttribute\n");
                        my $snapshotAttribute = $describeSnapshotAttributeResult->getSnapshotAttribute();
                        if ($snapshotAttribute->isSetSnapshotId()) 
                        {
                            print("                    SnapshotId\n");
                            print("                        " . $snapshotAttribute->getSnapshotId() . "\n");
                        }
                        my $createVolumePermissionList = $snapshotAttribute->getCreateVolumePermission();
                        foreach (@$createVolumePermissionList) {
                            my $createVolumePermission = $_;
                            print("                    CreateVolumePermission\n");
                            if ($createVolumePermission->isSetUserId()) 
                            {
                                print("                        UserId\n");
                                print("                            " . $createVolumePermission->getUserId() . "\n");
                            }
                            if ($createVolumePermission->isSetGroupName()) 
                            {
                                print("                        GroupName\n");
                                print("                            " . $createVolumePermission->getGroupName() . "\n");
                            }
                        }
                    } 
                } 

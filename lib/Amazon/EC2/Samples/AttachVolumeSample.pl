...
use lib qw(../../../.);  
 ...
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::AttachVolumeRequest
  ...
  sub invokeAttachVolume {
      ...
      eval {
              my $response = $service->attachVolume($request);
                ...
                print("        AttachVolumeResponse\n");
                if ($response->isSetAttachVolumeResult()) { 
                    print("            AttachVolumeResult\n");
                    my $attachVolumeResult = $response->getAttachVolumeResult();
                    if ($attachVolumeResult->isSetAttachment()) { 
                        print("                Attachment\n");
                        my $attachment = $attachVolumeResult->getAttachment();

...
use lib qw(../../../.);  
 ...
 # sample for Create Image Action
 #**********************************************************************/
 use Amazon::EC2::Model::CreateImageRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::CreateImageRequest
 # object or hash of parameters
 # invokeCreateImage($service, $request);

                                                        
    # 
    # Create Image Action Sample
    #
  sub invokeCreateImage {
      ...
      eval {
              my $response = $service->createImage($request);
                ...
                if ($response->isSetCreateImageResult()) { 
                    print("            CreateImageResult\n");
                    my $createImageResult = $response->getCreateImageResult();
                    if ($createImageResult->isSetImageId()) 
                    {
                        print("                ImageId\n");
                        print("                    " . $createImageResult->getImageId() . "\n");
                    }
                } 

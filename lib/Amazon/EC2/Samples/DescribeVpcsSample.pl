...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeVpcsRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeVpcsRequest
 ...
 # invokeDescribeVpcs($service, $request);
    ...
    #
  sub invokeDescribeVpcs {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->describeVpcs($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        DescribeVpcsResponse\n");
                ...
                if ($response->isSetDescribeVpcsResult()) { 
                    print("            DescribeVpcsResult\n");
                    my $describeVpcsResult = $response->getDescribeVpcsResult();
                    my $vpcList = $describeVpcsResult->getVpc();
                    foreach (@$vpcList) {
                        my $vpc = $_;
                        print("                Vpc\n");
                        if ($vpc->isSetVpcId()) 

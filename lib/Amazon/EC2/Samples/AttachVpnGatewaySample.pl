...
use lib qw(../../../.);  
 ...
 # sample for Attach Vpn Gateway Action
 #**********************************************************************/
 use Amazon::EC2::Model::AttachVpnGatewayRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::AttachVpnGatewayRequest
 # object or hash of parameters
 # invokeAttachVpnGateway($service, $request);

                                                                                                                                                                                                                                                                                        
    # 
    # Attach Vpn Gateway Action Sample
    #
  sub invokeAttachVpnGateway {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->attachVpnGateway($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        AttachVpnGatewayResponse\n");
                ...
                if ($response->isSetAttachVpnGatewayResult()) { 
                    print("            AttachVpnGatewayResult\n");
                    my $attachVpnGatewayResult = $response->getAttachVpnGatewayResult();
                    if ($attachVpnGatewayResult->isSetVpcAttachment()) { 
                        print("                VpcAttachment\n");
                        my $vpcAttachment = $attachVpnGatewayResult->getVpcAttachment();

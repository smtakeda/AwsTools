...
use lib qw(../../../.);  
 ...
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::TerminateInstancesRequest
  ...
  sub invokeTerminateInstances {
      ...
      eval {
              my $response = $service->terminateInstances($request);
                ...
                print("        TerminateInstancesResponse\n");
                ...
                if ($response->isSetTerminateInstancesResult()) { 
                    print("            TerminateInstancesResult\n");
                    ...
                    foreach (@$terminatingInstanceList) {
                        ...
                        {
                            print("                    InstanceId\n");
                            print("                        " . $terminatingInstance->getInstanceId() . "\n");
                        }
                        if ($terminatingInstance->isSetCurrentState()) { 
                            print("                    CurrentState\n");
                            my $currentState = $terminatingInstance->getCurrentState();
                        ...
                        if ($terminatingInstance->isSetPreviousState()) { 
                            print("                    PreviousState\n");
                            my $previousState = $terminatingInstance->getPreviousState();

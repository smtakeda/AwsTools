...
#
# Create Spot Datafeed Subscription  Sample
#
...
use lib qw(../../../.);  
 ...
 # sample for Create Spot Datafeed Subscription Action
 #**********************************************************************/
 use Amazon::EC2::Model::CreateSpotDatafeedSubscriptionRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::CreateSpotDatafeedSubscriptionRequest
 # object or hash of parameters
 # invokeCreateSpotDatafeedSubscription($service, $request);

                                                                                                                                                                                                                                                                                                                                                                        
    # 
    # Create Spot Datafeed Subscription Action Sample
    #
  sub invokeCreateSpotDatafeedSubscription {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->createSpotDatafeedSubscription($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        CreateSpotDatafeedSubscriptionResponse\n");
                ...
                if ($response->isSetCreateSpotDatafeedSubscriptionResult()) { 
                    print("            CreateSpotDatafeedSubscriptionResult\n");
                    my $createSpotDatafeedSubscriptionResult = $response->getCreateSpotDatafeedSubscriptionResult();
                    if ($createSpotDatafeedSubscriptionResult->isSetSpotDatafeedSubscription()) { 
                        print("                SpotDatafeedSubscription\n");
                        my $spotDatafeedSubscription = $createSpotDatafeedSubscriptionResult->getSpotDatafeedSubscription();

...
use lib qw(../../../.);  
 ...
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::StartInstancesRequest
      ...
      eval {
              my $response = $service->startInstances($request);
                ...
                if ($response->isSetStartInstancesResult()) { 
                    print("            StartInstancesResult\n");
                    my $startInstancesResult = $response->getStartInstancesResult();
                    my $startingInstancesList = $startInstancesResult->getStartingInstances();
                    foreach (@$startingInstancesList) {
                        my $startingInstances = $_;
                        print("                StartingInstances\n");
                        if ($startingInstances->isSetInstanceId()) 
                        {
                            print("                    InstanceId\n");
                            print("                        " . $startingInstances->getInstanceId() . "\n");
                        }
                        if ($startingInstances->isSetCurrentState()) { 
                            print("                    CurrentState\n");
                            my $currentState = $startingInstances->getCurrentState();
                        ...
                        if ($startingInstances->isSetPreviousState()) { 
                            print("                    PreviousState\n");
                            my $previousState = $startingInstances->getPreviousState();

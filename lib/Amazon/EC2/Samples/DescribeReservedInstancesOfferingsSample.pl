...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeReservedInstancesOfferingsRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeReservedInstancesOfferingsRequest
 ...
 # invokeDescribeReservedInstancesOfferings($service, $request);
    ...
    #
  sub invokeDescribeReservedInstancesOfferings {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->describeReservedInstancesOfferings($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        DescribeReservedInstancesOfferingsResponse\n");
                ...
                if ($response->isSetDescribeReservedInstancesOfferingsResult()) { 
                    print("            DescribeReservedInstancesOfferingsResult\n");
                    my $describeReservedInstancesOfferingsResult = $response->getDescribeReservedInstancesOfferingsResult();
                    my $reservedInstancesOfferingList = $describeReservedInstancesOfferingsResult->getReservedInstancesOffering();
                    foreach (@$reservedInstancesOfferingList) {
                        my $reservedInstancesOffering = $_;
                        print("                ReservedInstancesOffering\n");
                        if ($reservedInstancesOffering->isSetReservedInstancesOfferingId()) 
                        {
                            print("                    ReservedInstancesOfferingId\n");
                            print("                        " . $reservedInstancesOffering->getReservedInstancesOfferingId() . "\n");
                        }
                        if ($reservedInstancesOffering->isSetInstanceType()) 
                        {
                            print("                    InstanceType\n");
                            print("                        " . $reservedInstancesOffering->getInstanceType() . "\n");
                        }
                        if ($reservedInstancesOffering->isSetAvailabilityZone()) 
                        {
                            print("                    AvailabilityZone\n");
                            print("                        " . $reservedInstancesOffering->getAvailabilityZone() . "\n");
                        }
                        if ($reservedInstancesOffering->isSetDuration()) 
                        {
                            print("                    Duration\n");
                            print("                        " . $reservedInstancesOffering->getDuration() . "\n");
                        }
                        if ($reservedInstancesOffering->isSetUsagePrice()) 
                        {
                            print("                    UsagePrice\n");
                            print("                        " . $reservedInstancesOffering->getUsagePrice() . "\n");
                        }
                        if ($reservedInstancesOffering->isSetFixedPrice()) 
                        {
                            print("                    FixedPrice\n");
                            print("                        " . $reservedInstancesOffering->getFixedPrice() . "\n");
                        }
                        if ($reservedInstancesOffering->isSetProductDescription()) 
                        {
                            print("                    ProductDescription\n");
                            print("                        " . $reservedInstancesOffering->getProductDescription() . "\n");
                        }
                    }
                } 

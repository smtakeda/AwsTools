...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::GetConsoleOutputRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::GetConsoleOutputRequest
 ...
 # invokeGetConsoleOutput($service, $request);
    ...
    #
  sub invokeGetConsoleOutput {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->getConsoleOutput($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        GetConsoleOutputResponse\n");
                ...
                if ($response->isSetGetConsoleOutputResult()) { 
                    print("            GetConsoleOutputResult\n");
                    my $getConsoleOutputResult = $response->getGetConsoleOutputResult();
                    if ($getConsoleOutputResult->isSetConsoleOutput()) { 
                        print("                ConsoleOutput\n");
                        my $consoleOutput = $getConsoleOutputResult->getConsoleOutput();
                        if ($consoleOutput->isSetInstanceId()) 
                        {
                            print("                    InstanceId\n");
                            print("                        " . $consoleOutput->getInstanceId() . "\n");
                        }
                        if ($consoleOutput->isSetTimestamp()) 
                        {
                            print("                    Timestamp\n");
                            print("                        " . $consoleOutput->getTimestamp() . "\n");
                        }
                        if ($consoleOutput->isSetOutput()) 
                        {
                            print("                    Output\n");
                            print("                        " . $consoleOutput->getOutput() . "\n");
                        }
                    } 
                } 

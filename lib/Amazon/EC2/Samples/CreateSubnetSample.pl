...
use lib qw(../../../.);  
 ...
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::CreateSubnetRequest
      ...
      eval {
              my $response = $service->createSubnet($request);
                ...
                if ($response->isSetCreateSubnetResult()) { 
                    print("            CreateSubnetResult\n");
                    my $createSubnetResult = $response->getCreateSubnetResult();

...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::DescribeRegionsRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::DescribeRegionsRequest
 ...
 # invokeDescribeRegions($service, $request);
    ...
    #
  sub invokeDescribeRegions {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->describeRegions($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        DescribeRegionsResponse\n");
                ...
                if ($response->isSetDescribeRegionsResult()) { 
                    print("            DescribeRegionsResult\n");
                    my $describeRegionsResult = $response->getDescribeRegionsResult();
                    my $regionList = $describeRegionsResult->getRegion();
                    foreach (@$regionList) {
                        my $region = $_;
                        print("                Region\n");
                        ...
                        {
                            print("                    RegionName\n");
                            print("                        " . $region->getRegionName() . "\n");
                        }
                        if ($region->isSetEndpoint()) 
                        {
                            print("                    Endpoint\n");
                            print("                        " . $region->getEndpoint() . "\n");
                        }
                    }
                } 

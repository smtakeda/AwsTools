...
use lib qw(../../../.);  
 ...
 # sample for Get Password Data Action
 #**********************************************************************/
 use Amazon::EC2::Model::GetPasswordDataRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::GetPasswordDataRequest
 # object or hash of parameters
 # invokeGetPasswordData($service, $request);

                                                                                                                                                                                    
    # 
    # Get Password Data Action Sample
    #
  sub invokeGetPasswordData {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->getPasswordData($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        GetPasswordDataResponse\n");
                ...
                if ($response->isSetGetPasswordDataResult()) { 
                    print("            GetPasswordDataResult\n");
                    my $getPasswordDataResult = $response->getGetPasswordDataResult();
                    if ($getPasswordDataResult->isSetPasswordData()) { 
                        print("                PasswordData\n");
                        my $passwordData = $getPasswordDataResult->getPasswordData();
                        if ($passwordData->isSetInstanceId()) 
                        {
                            print("                    InstanceId\n");
                            print("                        " . $passwordData->getInstanceId() . "\n");
                        }
                        if ($passwordData->isSetTimestamp()) 
                        {
                            print("                    Timestamp\n");
                            print("                        " . $passwordData->getTimestamp() . "\n");
                        }
                        if ($passwordData->isSetData()) 
                        {
                            print("                    Data\n");
                            print("                        " . $passwordData->getData() . "\n");
                        }
                    } 
                } 

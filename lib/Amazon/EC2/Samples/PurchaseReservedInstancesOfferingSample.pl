...
use lib qw(../../../.);  
 ...
 #**********************************************************************/
 use Amazon::EC2::Model::PurchaseReservedInstancesOfferingRequest;
 # @TODO: set request. Action can be passed as Amazon::EC2::Model::PurchaseReservedInstancesOfferingRequest
 ...
 # invokePurchaseReservedInstancesOffering($service, $request);
    ...
    #
  sub invokePurchaseReservedInstancesOffering {
      my ($service, $request) = @_;  
      eval {
              my $response = $service->purchaseReservedInstancesOffering($request);
              
                print ("Service Response\n");
                print ("=============================================================================\n");

                print("        PurchaseReservedInstancesOfferingResponse\n");
                ...
                if ($response->isSetPurchaseReservedInstancesOfferingResult()) { 
                    print("            PurchaseReservedInstancesOfferingResult\n");
                    my $purchaseReservedInstancesOfferingResult = $response->getPurchaseReservedInstancesOfferingResult();
                    if ($purchaseReservedInstancesOfferingResult->isSetReservedInstancesId()) 
                    {
                        print("                ReservedInstancesId\n");
                        print("                    " . $purchaseReservedInstancesOfferingResult->getReservedInstancesId() . "\n");
                    }
                } 
